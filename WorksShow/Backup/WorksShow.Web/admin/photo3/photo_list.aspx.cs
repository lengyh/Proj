...
{
    public partial class photo_list : WorksShow.Web.UI.ManagePage
    {
        ...
        protected void btnPassed_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                {
                    //bll.Delete(this.channel_id, id);
                    string sql = "update dt_photo_attribute_value set content='通过' where photo_id=" + id + " and  attribute_id =17";
                    DbHelperOleDb.ExecuteSql(sql);
                }
            }
            JscriptMsg("批量审核成功啦！", Utils.CombUrlTxt("photo_list.aspx", "channel_id={0}&category_id={1}&keywords={2}&property={3}",
                this.channel_id.ToString(), this.category_id.ToString(), this.keywords, this.property), "Success");
        }
