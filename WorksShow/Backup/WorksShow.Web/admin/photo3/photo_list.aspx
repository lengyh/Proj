<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="photo_list.aspx.cs" Inherits="WorksShow.Web.admin.photo3.photo_list" %>
...
<body class="mainbody">
    ...
    <div class="tools_box">
        <div class="tools_bar">
            ...
            <a href="photo_edit.aspx?action=<%=ActionEnum.Add %>&channel_id=<%=this.channel_id %>"
                class="tools_btn"><span><b class="add">添加账户</b></span></a>
            ...
            <asp:LinkButton ID="LinkButton1" runat="server" CssClass="tools_btn" OnClientClick="return ExePostBack2('LinkButton1');"
                OnClick="btnPassed_Click"><span><b class="delete">批量审核通过</b></span></asp:LinkButton>
        </div>
        <div class="select_box">
            请选择：<asp:DropDownList ID="ddlCategoryId" runat="server" CssClass="select2" AutoPostBack="True"
            ...
            <asp:DropDownList ID="ddlProperty" runat="server" CssClass="select2" AutoPostBack="True"
                OnSelectedIndexChanged="ddlProperty_SelectedIndexChanged" Enabled="false">
            ...
            <asp:ImageButton ID="ibtnViewTxt" runat="server" ImageUrl="../images/ico-show-txt.png"
                ToolTip="文字列表视图" OnClick="ibtnViewTxt_Click" />
            <asp:ImageButton ID="ibtnViewImg" runat="server" ImageUrl="../images/ico-show-img.png"
                ToolTip="图像列表视图" OnClick="ibtnViewImg_Click" />
        </div>
    </div>
    <!--列表展示.开始-->
    <asp:Repeater ID="rptList1" runat="server" OnItemCommand="rptList_ItemCommand">
        ...
        <ItemTemplate>
            <tr>
                ...
                <td>
                    <%#new WorksShow.BLL.category().GetTitle(Convert.ToInt32(Eval("category_id")))%>
                </td>
                <td>
                ...
                <td align="center">
                    <a href="photo_edit.aspx?channel_id=<%#this.channel_id %>&action=<%#ActionEnum.Edit %>&id=<%#Eval("id")%>">
                        修改</a> <a href="printweituoshu.aspx?id=<%#Eval("id")%>" target="_blank">
                            打印</a>
                </td>
            </tr>
        </ItemTemplate>
        ...
        <ItemTemplate>
            <li>
                <div class="box">
                    ...
                    <dl>
                        <dt><a href="photo_edit.aspx?channel_id=<%#this.channel_id %>&action=<%#ActionEnum.Edit %>&id=<%#Eval("id")%>">
                            <%#Eval("title")%></a></dt>
                        <dd>
                        </dd>
                        <dd>
                            发布时间：<%#string.Format("{0:g}",Eval("add_time"))%></dd>
                        <dd class="btns">
                            <span class="right"><a href="photo_edit.aspx?channel_id=<%#this.channel_id %>&action=<%#ActionEnum.Edit %>&id=<%#Eval("id")%>">
                                <img src="../images/ico-6.png" style="vertical-align: middle;" title="修改" /></a>
                                &nbsp;&nbsp;<a href="printweituoshu.aspx?id=<%#Eval("id")%>" target="_blank">
                                打印</a>
                            </span>排序：<asp:TextBox ID="txtSortId" runat="server" Text='<%#Eval("sort_id")%>'
                                CssClass="txtInput2 small2" onkeypress="return (/[\d]/.test(String.fromCharCode(event.keyCode)));" />
                        </dd>
                    </dl>
                    <div class="clear">
                    </div>
