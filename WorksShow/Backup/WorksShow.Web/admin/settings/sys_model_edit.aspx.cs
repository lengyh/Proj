...
{
    public partial class sys_model_edit : WorksShow.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == ActionEnum.Edit.ToString())
            {
                    ...
                    if (!new WorksShow.BLL.sys_model().Exists(this.id))
                    {
        ...
        {
            WorksShow.BLL.sys_model bll = new BLL.sys_model();
            WorksShow.Model.sys_model model = bll.GetModel(_id);
        ...
        {
            bool result = true;
            WorksShow.Model.sys_model model = new Model.sys_model();
            WorksShow.BLL.sys_model bll = new BLL.sys_model();
            ...
            if (!string.IsNullOrEmpty(nav_title) && !string.IsNullOrEmpty(nav_url) && !string.IsNullOrEmpty(nav_sort))
            {
                try
                {
                    ...
                    List<WorksShow.Model.sys_model_nav> ls = new List<Model.sys_model_nav>();
                    for (int i = 0; i < titleArr.Length; i++)
                    {
                        ls.Add(new WorksShow.Model.sys_model_nav { title = titleArr[i], nav_url = urlArr[i], sort_id = int.Parse(sortArr[i].Trim()) });
                    }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (!string.IsNullOrEmpty(nav_id) && !string.IsNullOrEmpty(nav_title) && !string.IsNullOrEmpty(nav_url) && !string.IsNullOrEmpty(nav_sort))
            {
                try
                {
                    ...
                    for (int i = 0; i < titleArr.Length; i++)
                    {
                        ls.Add(new WorksShow.Model.sys_model_nav { id = int.Parse(idArr[i]), model_id = model.id, title = titleArr[i], nav_url = urlArr[i], sort_id = int.Parse(sortArr[i].Trim()) });
                    }
                    model.sys_model_navs = ls;
                }
                catch
