...
function autoPlay(view, NF) {
	if (view == 0) {
		var sId = setInterval(function () {
			document.getElementsByClassName("arrow-right")[0].click();
			if (parseInt(now_img) + 1 == total_img) {
				// alert("now:"+now_img+1+"\ntotal:"+total_img);
				clearInterval(sId);
				autoPlay(parseInt(now_img) + 1, total_img);
			}
			// alert("now:"+parseInt(parseInt(now_img)+1)+"\ntotal:"+total_img);
		}, 3000);
		cycleId=sId;
	} else if (view == NF) {
		var sIdd = setInterval(function () {
			document.getElementsByClassName("arrow-left")[0].click();
			if (now_img == 0) {
				clearInterval(sIdd);
				autoPlay(now_img, total_img);
			}
		}, 3000);
		cycleId=sIdd;
	} else {
		var sIddd = setInterval(function () {
			document.getElementsByClassName("arrow-left")[0].click();
			if (now_img == 0) {
				clearInterval(sIddd);
				autoPlay(now_img, total_img);
