...
namespace WorksShow.Web.admin.photo3
{
    public partial class photo_edit : WorksShow.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == ActionEnum.Edit.ToString())
            {
                ...
                if (!new WorksShow.BLL.photo().Exists(this.id))
                {
        ...
        {
            bool result = true;
            WorksShow.Model.photo model = new Model.photo();
            WorksShow.BLL.photo bll = new BLL.photo();

            model.channel_id = this.channel_id;
            model.title = txtTitle.Text.Trim();
            model.category_id = int.Parse(ddlCategoryId.SelectedValue);
            model.photo_no = txtPhotoNo.Text.Trim();
            model.market_price = 20;//decimal.Parse(txtMarketPrice.Text.Trim());
            model.sell_price = 20;// decimal.Parse(txtSellPrice.Text.Trim());
            ...
            model.sort_id = 20;//int.Parse(txtSortId.Text.Trim());
            model.click = 20;// int.Parse(txtClick.Text.Trim());
            model.is_msg = 0;
            model.is_top = 0;
            model.is_red = 0;
            model.is_hot = 0;
            ...
            attrls.Add(new Model.photo_attribute_value { attribute_id = 17, title = "账户审核", content = "未通过" });
        ...
        private bool DoEdit(int _id)
        {
            bool result = true;
            WorksShow.BLL.photo bll = new BLL.photo();
            WorksShow.Model.photo model = bll.GetModel(_id);
