...
namespace WorksShow.DAL
{
    ...
    public partial class category
    {
        ...
        {
            string strSql = "select top 1 id from dt_category order by id desc";
            ...
            using (OleDbConnection conn = new OleDbConnection(DbHelperOleDb.connectionString))
            {
                ...
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        //修改节点列表和深度
                        DbHelperOleDb.ExecuteSql(conn, trans, "update dt_category set class_list='" + model.class_list + "', class_layer=" + model.class_layer + " where id=" + model.id); //带事务
                        trans.Commit();
                    }
                    catch
                    {
                ...
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (IsContainNode(model.id, model.parent_id))
                        {
                            ...
                            //先提升选中的父节点
                            DbHelperOleDb.ExecuteSql(conn, trans, "update dt_category set parent_id=" + oldModel.parent_id + ",class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + model.parent_id); //带事务
        ...
        public WorksShow.Model.category GetModel(int id)
        {
            ...
            WorksShow.Model.category model = new WorksShow.Model.category();
        ...
        /// 得到一个对象实体(重载，带事务)
        /// </summary>
        public WorksShow.Model.category GetModel(OleDbConnection conn, OleDbTransaction trans, int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,channel_id,title,parent_id,class_list,class_layer,sort_id,link_url,img_url,content,seo_title,seo_keywords,seo_description from dt_category ");
            strSql.Append(" where id=@id");
        ...
        private void UpdateChilds(OleDbConnection conn, OleDbTransaction trans, int parent_id)
        {
            ...
            if (model != null)
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    DbHelperOleDb.ExecuteSql(conn, trans, "update dt_category set class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + id); //带事务
        ...
        public DataSet GetTermList(string parentID,bool includeSelf = false)
        {
            ...
            {
                strSql.Append(" where parent_id=" + parentID);
                if(includeSelf)
                {
                    strSql.Append(" or id=" + parentID);
                }
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,parent_id,title from dt_category");
            if (!string.IsNullOrEmpty(parentID))
            {
                strSql.Append(" where parent_id=" + parentID);
            }
            ...
            {
                int a =int.Parse(ds.Tables[0].Rows[i][0].ToString());
                list.Add(a);
            }
        ...
        public int GetWorkInfo(string parentID, string title)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,parent_id,title from dt_category");
            if (!string.IsNullOrEmpty(parentID) && !string.IsNullOrEmpty(title))
            {
                strSql.Append(" where parent_id=" + parentID +" and title='"+title+"'");
            }
            strSql.Append(" order by id asc");
            DataSet ds = DbHelperOleDb.Query(strSql.ToString());
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,parent_id,title,img_url from dt_category");
            if (!string.IsNullOrEmpty(termId))
            {
                strSql.Append(" where parent_id=" + termId);
            }
            strSql.Append(" order by id asc");
            DataSet ds = DbHelperOleDb.Query(strSql.ToString());
